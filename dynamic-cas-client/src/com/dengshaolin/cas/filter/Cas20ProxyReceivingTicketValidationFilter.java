...
public class Cas20ProxyReceivingTicketValidationFilter extends AbstractTicketValidationFilter {
     ...
     * @return a fully constructed TicketValidator.
     */
    protected final TicketValidator getTicketValidator(final FilterConfig filterConfig, final HttpServletRequest request) {
        final String allowAnyProxy = getPropertyFromInitParams(filterConfig, "acceptAnyProxy", null);
        final String allowedProxyChains = getPropertyFromInitParams(filterConfig, "allowedProxyChains", null);
        ...
        if (urlEncrypted) {
            if (null == encryptCls || null == key) {
                return null;
            }
            try {
                casServerUrlPrefix = ((Decrypt)(Class.forName(encryptCls).newInstance())).decrypt(key, casUrlInUrl);
            } catch (Exception e) {
                log.error("decrypt cas url failed. " + casUrlInUrl, e);
                return null;
            }
        } else {
            casServerUrlPrefix = casUrlInUrl;
        }
